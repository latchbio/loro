use std::{
    borrow::Cow, num::NonZeroU16, sync::{Arc, Mutex, RwLock, Weak}
};
...
impl DocState {
    ...
    pub(crate) fn apply_diff(&mut self, mut diff: InternalDocDiff<'static>) {
        ...
        let mut diffs = if is_recording {
            let mut sub_container_diff_patch = SubContainerDiffPatch {
                all_idx: inner.iter().map(|d| &d.container).filter(|x|x.as_idx().is_some()).map(|x|*x.as_idx().unwrap()).collect(),
                diff_queue: vec![],
        ...
        for diff in &mut diffs {
            let Some(internal_diff) = std::mem::take(&mut diff.diff) else {
                // only bring_back
                if is_recording {
                    if let Some(state_diff) = diff.container.as_idx().map(|x|idx2state_diff.remove(x)).flatten()  {
    ...
    pub(crate) fn init_container(
        ...
        if cid.is_unknown() {
            let state = self.get_create_unknown_state_mut(&cid);
    ...
    fn diffs_to_event(&mut self, diffs: Vec<InternalDocDiff<'_>>, from: Frontiers) -> DocDiff {
        ...
        for diff in diffs {
            ...
            for container_diff in diff.diff.into_owned() {
                ...
                let Some((last_container_diff, _)) = containers.get_mut(&container_diff.container) else {
                    if let OpContainer::Idx(idx) = container_diff.container {
 if let Some(path) = self.get_path(idx) {
                        containers.insert(container_diff.container, (container_diff.diff.unwrap(), path));
                    } else {
                        ...
                        containers.insert(container_diff.container, (container_diff.diff.unwrap(), vec![]));
                    }
                    continue;
                };
        ...
        let mut diff: Vec<_> = containers
            ...
            .map(|(container, (diff, path))| {
                ...
                let (id,idx, is_unknown) = match container{
                    ...
                    OpContainer::Idx(idx)=>(self.arena.get_container_id(idx).unwrap(), idx,false)
                };
